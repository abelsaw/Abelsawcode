# Viral watch — usage history

This is the rolling ledger of stories covered in past `/viral-watch` runs. The
skill reads it before researching each new run and excludes story URLs from
the **last 3 runs** to avoid duplication. To allow a story to recycle (e.g.
when a slow-burn story re-surges), manually delete its entry from the relevant
run below.
