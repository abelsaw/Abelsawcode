#!/usr/bin/env python3
"""hr-linkedin-option3 dark conclusion — slide 7 of 7.

Closing slide on charcoal background — the only dark slide in the
carousel. Same two-part body shape as the content slides, inverted
for legibility on dark.

Palette: charcoal BG + cream text + tan slide # + cream rule.
"""
import argparse
import sys
...
BG = (42, 42, 51)             # charcoal (dark BG)
CREAM = (235, 225, 208)       # inverted text color
TAN = (181, 143, 96)
...
def main() -> int:
    ...
    # --- Top-left: LARGE slide # in tan with thin cream rule ---
    ...
    if sn is not None:
        ...
        rule_y = nbbox[3] + 12
        draw.rectangle([pad, rule_y, pad + 110, rule_y + 4], fill=CREAM)
