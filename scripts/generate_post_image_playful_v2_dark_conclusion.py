#!/usr/bin/env python3
"""hr-linkedin-option2 dark conclusion — slide 7 of 7.

Closing slide on navy. Bridges to the cream content slides via a thin
mustard "wayfinder residue" stripe at the top — same color as the
GROW tile from the cover, signaling the carousel's resolution.

Palette: navy BG + cream text + coral slide # + mustard accent stripe.
"""
import argparse
import sys
...
def main() -> int:
    ...
    # --- Top: mustard wayfinder residue stripe (full content width) ---
    stripe_y = pad
    stripe_h = 14
    draw.rectangle([pad, stripe_y, pad + content_w, stripe_y + stripe_h],
                   fill=MUSTARD)
    top_block_end = stripe_y + stripe_h

    # --- Slide # large coral with no rule (the stripe already serves) ---
    sn, st = parse_slide(args.slide)
    head_top_min = top_block_end + 40
    if sn is not None:
        num_font = load_font(FONT_SANS_BOLD, 92)
        num_text = f"{sn:02d}"
        draw.text((pad - 6, head_top_min), num_text, font=num_font, fill=CORAL)
        nbbox = draw.textbbox((pad - 6, head_top_min), num_text, font=num_font)
        head_top_min = nbbox[3] + 30
    ...
    # --- Headline ---
    head_max_h = int((bottom_source_y - head_top_min) * 0.46)
